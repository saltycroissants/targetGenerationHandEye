...
namespace Tobii.XR
{
    public static class CoordinatesHelper
    {
        ...
        /// <summary>
        /// Unity saves the camera world transform when application starts and automatically applies it to the camera before applying head pose to it.
        /// This method calculates this transform by getting the difference between head pose (CenterEye) and final camera transform.
        /// </summary>
        /// <returns></returns>
        public static Matrix4x4 GetCenterEyeToCameraTransform()
        {
            var cam = CameraHelper.GetMainCamera();

            // Get current center eye pose
            Vector3 centerEyePosition;
            Quaternion centerEyeRotation;

            InputTracking.GetNodeStates(TempNodeStates);
            var centerEyeNodeIndex = TempNodeStates.FindIndex(x => x.nodeType == XRNode.CenterEye);
            if (centerEyeNodeIndex < 0) return Matrix4x4.identity;

            var centerEyeNode = TempNodeStates[centerEyeNodeIndex];
            if (!centerEyeNode.TryGetPosition(out centerEyePosition))
                return Matrix4x4.identity;
            if (!centerEyeNode.TryGetRotation(out centerEyeRotation))
                return Matrix4x4.identity;

            // Create a matrix describing the transform from center eye to camera. Mx*Mce=Mcam
            return cam.transform.localToWorldMatrix * Matrix4x4.TRS(centerEyePosition, centerEyeRotation, Vector3.one).inverse;
        }
        ...
        {
            if (_leftOffset.HasValue && _rightOffset.HasValue)
            {
                leftOffset = _leftOffset.Value;
                rightOffset = _rightOffset.Value;
                return true;
            }
            ...
            if (leftEye.HasValue && rightEye.HasValue && centerEye.HasValue)
            {
                ...
                _leftOffset = leftOffset = new Vector3(leftMat.m03, leftMat.m13, leftMat.m23);
                _rightOffset = rightOffset = new Vector3(rightMat.m03, rightMat.m13, rightMat.m23);
                return true;
            }
