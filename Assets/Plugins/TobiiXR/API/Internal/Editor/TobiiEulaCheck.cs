...
and agree to be bound, by the Tobii XR Unity SDK License and Use Agreement.</color></size>";
        ...
        private static TobiiEulaFile LoadOrCreateEulaFile()
        {
            ...
            if (!resourceExists)
            {
                var facadePath = PathHelper.FindPathToClass(typeof(TobiiXR));
                var sdkPath = string.IsNullOrEmpty(facadePath) ? "Assets/TobiiXR/API" : Path.GetDirectoryName(facadePath);
